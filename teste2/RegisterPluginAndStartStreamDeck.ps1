...
$pluginProjectFile = "$basePath\plugins-stream-deck.csproj"
...
If (-not (Test-Path $bindir)) {
  Write-Error "The output directory `"$bindir`" was not found.`n You must first build the `"plugins_stream_deck`" project before calling this script.";
  exit 1;
}
...
$pluginID = $uuidAction.substring(0, $uuidAction.Length - ".DefaultPluginAction".Length)
